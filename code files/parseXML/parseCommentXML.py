    ...
    for token in token_list:
        if token not in stop_words and token not in string.punctuation:
            ...
            if not token.isdigit():
                if token != "==" and token != "{2021}r" and len(token)>2 and check_token(token) and check_pos_tag(token) and (token not in word_list):       
                    if nltk.pos_tag([token])[0][1] == 'JJ':
                        jj_list = token.split('-') 
...
if __name__ == "__main__":
    ...
    with open('comment_tagged_tokens.csv', 'w') as myfile:      #changed wb to w
        wr = csv.writer(myfile, delimiter = '\n')
        wr.writerow(all_tagged_comment_tokens)


    with open('comment_tokens.csv', 'w') as myfile:             #changed wb to w
        wr = csv.writer(myfile, delimiter = '\n')
        wr.writerow(all_comment_tokens)

    pickle.dump( all_comment_tokens, open( "../../Data files/curl_comment_tokens.p", "wb" ) )
