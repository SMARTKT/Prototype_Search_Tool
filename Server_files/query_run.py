...
#nltk.download('punkt')         #remove hash later
#nltk.download('stopwords')     #remove hash later
ps = PorterStemmer() 
...
class QueryConversion:
    ...
    def __init__(self,all_store_pickle,all_files_pickle,name_tokens_pickle,comment_tokens_pickle,program_domain_pickle,tf_idf_name_tokens_pickle,tf_idf_symbols_pickle):
        ...
        self.mapping = {
                        ...
                        22:self.template_static_attribute_dynamic_attr_symbol
                        }
        '''
                ...
                for comment_token in self.all_comment_tokens_dict.keys():
                    for similar_token in self.all_comment_tokens_dict[comment_token]:
                        if re.match('\\b'+str(word)+'\\b',str(similar_token[0]), re.IGNORECASE) or re.match('\\b'+str(ps.stem(word))+'\\b',str(similar_token[0])):  #so that 'many' doesn't match 'manytomany' or 'manipul'
                            if float(similar_token[1]) > float(maxx):
    ...
    def template_attr_symbol(self,word_store,graph):
        ...
        if(len(word_store['dynamic_attribute']) > 0):
            ans = self.template_static_attribute_dynamic_attr_symbol(word_store,graph)
            return ans
    ...
    def template_common_file(self,word_store,graph):
        ...
        query_part_6 = """
            FILTER (?pred IN (prop:storage_class, prop:is_defined_file, prop:type, prop:spelling, prop:is_a, prop:isDef_file_line, prop:isDecl_file_line, prop:linkage_kind, prop:isUse_file_line, prop:is_array, prop:array_size, prop:is_pointer, prop:is_subclass_of, prop:is_friend_of) ) . 
            }
            """
        ...
        for ind,matched_symbol in enumerate(word_store['fil']):
            ...
            for matched_word in word_store['common_word']:
                ...
                for tag in tag_list:
                    ...
                    for row in result:
                        curr_id = row['id'].split('#')[1]
                        pred_val = str(row['pred'].split('#')[1])
                        if pred_val == "isUse_file_line":
                            if pred_val in result_dict[curr_id].keys():
                                result_dict[curr_id][pred_val].add(str(row['obj']))
                            else:
                                temp = set()
                                temp.add(str(row['obj']))
                                result_dict[curr_id][pred_val] = temp

                        else:
                            result_dict[curr_id][pred_val] = str(row['obj'])
        ...
        for key in result_dict:
            score = 1
            if 'linkage_kind' in result_dict[key]:
                if result_dict[key]['linkage_kind'] == "external":
                    score += 10000
            if 'isUse_file_line' in result_dict[key]:
                score += len(result_dict[key]['isUse_file_line'])
            scores[key] = score 
        ...
        for key in sorted(scores, key=scores.get, reverse=True):
            ...
            if 'is_a' in result_dict[key]:
                ans += 'Identifier type : ' + result_dict[key]['is_a'] + '\n'
            if 'isDef_file_line' in result_dict[key]:
                ans += 'Definition File Line : ' + result_dict[key]['isDef_file_line'] + '\n'  
            if 'isUse_file_line' in result_dict[key]:
                use_files = ""
                for file in result_dict[key]['isUse_file_line']:
                    use_files += file + ",  "
                use_files = use_files[:-2]
                ans += 'Usage File Line : ' + use_files + '\n'      
            if 'isDecl_file_line' in result_dict[key]:
                ans += 'Declaration File Line : ' + result_dict[key]['isDecl_file_line'] + '\n'
            if 'linkage_kind' in result_dict[key]:
                ans += 'Linkage Kind : ' + result_dict[key]['linkage_kind'] + '\n\n'      
    ...
    def template_dynamic_attr_file(self,word_store,graph):
        ...
        query_part_6 = """
            FILTER (?pred1 IN (prop:storage_class, prop:type, prop:spelling, prop:is_a, prop:isDef_file_line, prop:linkage_kind, prop:isUse_file_line) ) .
            }
            """                     
        ...
        for row in resultFirst:
            curr_id = row['id'].split('#')[1]
            pred_val = str(row['pred1'].split('#')[1])
            # if curr_id in result_dict:
            if pred_val == "isUse_file_line":
                if pred_val in result_dict[curr_id].keys():
                    result_dict[curr_id][pred_val].add(str(row['obj1']))
                else:
                    temp = set()
                    temp.add(str(row['obj1']))
                    result_dict[curr_id][pred_val] = temp

            else:
                result_dict[curr_id][pred_val] = str(row['obj1'])
    ...
    def template_value_common_dynamic_attr_file(self,word_store,graph):
        ...
        query_part_6 = """
            FILTER (?pred1 IN (prop:storage_class, prop:type, prop:spelling, prop:is_a, prop:isDef_file_line, prop:isDecl_file_line, prop:linkage_kind, prop:isUse_file_line ) ) .
            }
            """                     
    ...
    #return type of child function of multi_runsingle				
    def template_static_attribute_dynamic_attr_symbol(self,word_store,graph):
        ...
        query = """
            ...
            SELECT DISTINCT ?id ?obj ?obj1
            WHERE
            {
                ?id prop:spelling ?obj.
                ?id prop:is_a "FunctionDecl".
                ?id prop:return_type ?obj1.
                {
                    SELECT ?obj
                    WHERE
                    {
                        ?seq prop:caller_name "%s".
                        ?seq prop:callee_name ?obj.
                    }
                }
            }
        """
        ...
        for stmt in resultFirst:
            print(stmt)

        print("Time taken to process query",+end_time-start_time)
        
        for row in resultFirst:
            curr_seq = row['id'].split('#')[1]
            if curr_seq in result:
                result[int(curr_seq)][str(row['obj'])] = str(row['obj1'])
            else:
                result[int(curr_seq)]['name'] = str(symbol_name)
                result[int(curr_seq)][str(row['obj'])] = str(row['obj1'])
